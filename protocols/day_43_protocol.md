# Day 43 - Unsupervised Learning: Dimensionality Reduction
...
7. [Common Mistakes & Solutions](#7-common-mistakes--solutions)
8. [Practical Tips & Workflow](#8-practical-tips--workflow)
9. [Advanced Topics & Extensions](#9-advanced-topics--extensions)
10. [Notebook Exercises Summary](#10-notebook-exercises-summary)
11. [Helpful References](#11-helpful-references)
...
**Result**: Beautiful visualization showing distinct clusters for each digit (0-9)!

---

## __PCA in ML Pipelines__
...
### Mistake 2: Using t-SNE in a Pipeline
```python
# ❌ Wrong - t-SNE has no transform method!
model = make_pipeline(TSNE(n_components=2), LogisticRegression())

# ✅ Correct - Use PCA for modeling
model = make_pipeline(PCA(n_components=50), LogisticRegression())
```

### Mistake 3: Fitting PCA on Entire Dataset
```python
# ❌ Wrong - Data leakage!
pca = PCA()
...
model = make_pipeline(StandardScaler(), PCA(), Classifier())
model.fit(X_train, y_train)
```

### Mistake 4: Interpreting t-SNE Distances
```python
# ❌ Wrong interpretation
"Cluster A and B are far apart in t-SNE, so they're very different"
# Distance between clusters in t-SNE is NOT meaningful!

# ✅ Correct interpretation
"t-SNE shows distinct clusters, suggesting class separability"
# Focus on cluster separation, not distances
```

### Mistake 5: Too Few Components
```python
# ❌ Potentially problematic
pca = PCA(n_components=2)  # Only for visualization!

# ✅ Better approach
pca = PCA(n_components=0.95)  # Keep 95% of variance
# Or check scree plot and choose informed number
```

---

## __Practical Tips & Workflow__
...
## __Advanced Topics & Extensions__
...
## __Notebook Exercises Summary__

### 1. Principal Component Analysis (Wine Quality Dataset)
**File**: `1_Principal_Component_Analysis.ipynb`

**What we did:**
* Loaded wine dataset: 178 samples, 13 features, 3 classes
* Explored data with box plots and correlation matrix
* Applied StandardScaler (critical step!)
* Fitted PCA with all components to analyze variance
* Compared classification accuracy:
  * Gaussian Naive Bayes on all 13 features: ~98-99%
  * Gaussian Naive Bayes on first 2 PCs: ~95-97%
* Visualized class separation with 2D scatter plots

**Key takeaway**: PCA achieves massive dimensionality reduction (13→2) with minimal accuracy loss

### 2. t-SNE for Visualization
**File**: `2_t_SNE.ipynb`

**What we did:**
* Compared PCA vs t-SNE on Iris dataset
* Applied t-SNE to 20 Newsgroups text data:
  * Pre-processed with TF-IDF (2,034 × 34,118 sparse matrix)
  * Reduced with TruncatedSVD to 50 dimensions
  * Applied t-SNE for 2D visualization
  * Visualized 4 document categories with clear clusters
* Applied t-SNE to MNIST handwritten digits:
  * Pre-reduced with PCA to 50 dimensions
  * Created beautiful 2D visualization of digit clusters
* Explored mathematical background (optional)

**Key takeaway**: t-SNE reveals non-linear structure and creates beautiful visualizations, but only for exploration

### 3. PCA in Pipeline for Face Recognition
**File**: `3_PCA_in_Pipeline.ipynb`

**What we did:**
* Loaded Labeled Faces in the Wild dataset
* Built pipeline: PCA(150) → SVC classifier
* Used GridSearchCV to tune:
  * `svc__C`: [5, 10, 50]
  * `svc__gamma`: [0.0005, 0.001, 0.005]
* Evaluated with classification report
* Visualized confusion matrix with seaborn
* Displayed predictions on test images

**Key takeaway**: Pipelines with PCA enable efficient high-dimensional classification while preventing data leakage
...
### Course Materials
* [Session Slides (PDF)](FireShot%20Capture%20020%20-%20Unsupervised%20Learning%20-%20Dimensionality%20Reduction%20—%20DS%20Course%20Materi_%20-%20[ideal-adventure-6vymekm.pages.github.io].pdf)
